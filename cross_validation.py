...
def cross_validate_kernel_grid(kernel_type, kernel_params, reg_params, X_train, y_train, quantiles, 
    ...
    for train_index, test_index in kf.split(X_train):
        ...
        for quantile in quantiles:
            ...
            s = len(quantiles) + 1
            cv_dir_name = cv_dir_name_base + '/cv_results_i'+ str(round(quantile * s)) + 's' + str(s)
...
def get_params(best_params, surrogate):
    ...
    for quantile in best_params.keys():
        quantiles.append(quantile)
        regs.append(best_params[quantile]['reg'])
        kernels.append(best_params[quantile]['kernel'])
